...
import { selectMowingUpgradeLevelValue } from '../../root-store/upgrades/upgrades-selector';
...
export class UpgradesService {
    ...
    private _unlockUpgradeSubscription: Subscription = this.doUnlockUpgrade$
        .pipe(
            withLatestFrom(this.store.select(selectMowingUpgradeLevelValue), this.money$, (id, upgrades, money) => {
                const upgrade: Upgrade = upgrades.find((u) => u.id == id);
                if (money >= upgrade.price(upgrade.level) && upgrade.level < upgrade.maxLevel) {
                    this.store.dispatch(unlockMowingUpgradeAction({ id }));
                    this.store.dispatch(EarningAction.earnMoney({ money: -upgrade.price(upgrade.level - 1) }));
                }
            }),
        )
        .subscribe();
